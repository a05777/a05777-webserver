...
* **Adaptive Protocol / 协议自适应**: Features original protocol sniffing logic, supporting **simultaneous HTTP & HTTPS on a single port** (automatically identifies the first byte for forwarding).
  独创协议嗅探逻辑，支持 **单端口同时兼容 HTTP & HTTPS**（自动识别首字节进行转发）。
...
* **Peak Throughput / 最高并发**: 11,728 RPS
```
...
Running 30s test @ https://node1-rn.a05777.uk:8443/
  ...
  Thread Stats   Avg      Stdev     Max   +/- Stdev
    Latency    79.43ms   47.01ms   1.57s    92.87%
    Req/Sec     2.00k   508.42     3.11k    71.35%
  Latency Distribution
     50%   67.56ms
     75%   87.51ms
     90%  112.25ms
     99%  325.87ms
  352945 requests in 30.09s, 0.96GB read
Requests/sec:  11728.62
